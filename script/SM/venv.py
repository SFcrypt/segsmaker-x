...
def install_venv(ui, url, need_space, vnv, fn):
    ...
    # Determinar directorio personal según UI
    dirs1 = ['A1111', 'Forge', 'ReForge', 'Forge-Classic', 'ComfyUI', 'SwarmUI', 'FaceFusion', 'SDTrainer']
    if ui in dirs1:
        personal_dir = HOME / ui
    else:
        personal_dir = HOME / 'default'
    
    # Crear directorio personal si no existe
    personal_dir.mkdir(parents=True, exist_ok=True)
    
    # Descargar en directorio personal
    tar_file = personal_dir / fn
    
    # Cambiar al directorio personal para la descarga
    CD(personal_dir)
    ...
    # Descomprimir en /tmp
    CD(tmp)
    SyS(f'pv "{tar_file}" | lz4 -d | tar xf -')
...
if not vnv.exists():
    install_venv(ui, url, need_space, vnv, fn)
